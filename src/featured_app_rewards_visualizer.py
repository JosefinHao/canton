"""
Featured App Rewards Visualizer

This module provides visualization capabilities for featured app rewards analysis,
including individual app progress charts, comparison charts, and ecosystem overviews.
"""
...
class FeaturedAppRewardsVisualizer:
    """
    Visualizer for featured app rewards data.
    """

    def __init__(self, analyzer: FeaturedAppRewardsAnalyzer):
        """
        Initialize the visualizer.

        Args:
            analyzer: FeaturedAppRewardsAnalyzer instance with processed data
        """
    ...
    def plot_app_progress(
        ...
        Returns:
            Output filename
        """
        stats = self.analyzer.get_provider_stats(provider_party_id)
        if not stats:
            raise ValueError(f"No data found for provider {provider_party_id}")
        ...
        provider_display = self._format_provider_id(provider_party_id, max_length=50)
        plt.title(f'Featured App Rewards Progress\n{provider_display}',
        ...
        # Save figure
        if not output_file:
            safe_name = provider_party_id.replace(':', '_').replace('/', '_')[:50]
    ...
    def plot_top_apps_comparison(
        ...
        """
        Plot comparison of top featured apps.
        ...
        if by_metric == 'rewards':
            top_apps = self.analyzer.get_top_apps_by_rewards(limit=limit)
            ylabel = 'Total Rewards (CC)'
            title_metric = 'Total Rewards'
        elif by_metric == 'activity':
            top_apps = self.analyzer.get_top_apps_by_activity(limit=limit)
        ...
        # Prepare data
        providers = [self._format_provider_id(p_id, 30) for p_id, _ in top_apps]
    ...
    def plot_app_comparison_timeline(
        ...
        for i, provider_id in enumerate(provider_ids):
            stats = self.analyzer.get_provider_stats(provider_id)
            if not stats:
                continue
            ...
            label = self._format_provider_id(provider_id, 40)
    ...
    def plot_ecosystem_overview(
        ...
        Args:
            output_file: Output filename
            top_n: Number of top apps to show individually (others grouped as "Other")

        Returns:
            Output filename
        """
        # Get top N apps by total rewards
        top_apps = self.analyzer.get_top_apps_by_rewards(limit=top_n)
        ...
        for provider_id in top_provider_ids:
            ...
            labels.append(self._format_provider_id(provider_id, 30))
        ...
        ax.set_title('Featured App Rewards Ecosystem Overview',
                    fontsize=14, fontweight='bold', pad=20)
    ...
    def plot_rewards_heatmap(
        ...
        Args:
            output_file: Output filename
            top_n: Number of top apps to include
        ...
        for provider_id, stats in top_apps:
            rewards_series = [stats.rewards_by_round.get(r, 0) for r in rounds]
            data_matrix.append(rewards_series)
            app_labels.append(self._format_provider_id(provider_id, 35))
        ...
        ax.set_title('Featured App Rewards Heatmap by Round',
                    fontsize=14, fontweight='bold', pad=20)
    ...
    def plot_reward_distribution(
        ...
        top_apps = self.analyzer.get_top_apps_by_rewards(limit=10)
        ...
        for provider_id, stats in top_apps:
            pie_labels.append(self._format_provider_id(provider_id, 25))
    ...
    def generate_report(
        ...
        print("  - Generating top apps by activity chart...")
        ...
        # 6. Timeline comparison of top apps
        print("  - Generating timeline comparison (per round)...")
        top_apps = self.analyzer.get_top_apps_by_rewards(limit=min(10, top_apps_limit))
        top_ids = [p_id for p_id, _ in top_apps]
        ...
        # 8. Individual progress charts for top apps
