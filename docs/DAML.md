...
**DAML (Digital Asset Modeling Language)** is a smart contract language where:

* **Templates** define contract types — the shape of state on the ledger

* **Choices** define actions parties can take — the transitions between states

* **Signatories** are parties whose consent is required for a contract to exist

* **Observers** are parties with read access to a contract

* **Controllers** are parties authorized to exercise a choice

The ledger records every action as events in an **update tree**: `created_event`, `exercised_event`, or `archived_event` (archive is a special consuming exercise). A choice that creates child contracts produces a tree of events.

## **Template Categories by Transaction Type**

### **1\. Canton Coin (Amulet) Transfers**

**`Splice.Amulet:Amulet`** — Core value container (analogous to a UTXO)

* Signatories: DSO \+ owner

* Fields: owner, dso, amount (`ExpiringAmount` — decays via holding fees each round)

* Choice: `Amulet_Expire` — DSO archives an expired amulet

**`Splice.Amulet:LockedAmulet`** — Amulet with a `TimeLock` (requires co-signers to unlock)

* Choices: `Unlock`, `OwnerExpireLock`, `ExpireAmulet`

* Used as escrow during payments, subscriptions, and two-step transfers

**`Splice.AmuletRules:AmuletRules`** — The master rulebook for all CC operations

* Controlled entirely by DSO

* Key choices:

  * `AmuletRules_Transfer` — batch transfer between parties, consuming input amulets and creating output amulets, computing tiered fees

  * `AmuletRules_ComputeFees` — dry-run fee calculation

  * `AmuletRules_BuyMemberTraffic` — convert CC into synchronizer bandwidth (traffic credits)

  * `AmuletRules_Mint` — mint new CC (DevNet only)

  * `AmuletRules_DevNet_Tap` — faucet (DevNet only)

**`Splice.AmuletRules:TransferPreapproval`** — Allows a receiver to pre-authorize incoming transfers (sender can push without receiver being online)

**`Splice.AmuletTransferInstruction:AmuletTransferInstruction`** — A two-step transfer primitive; locks amulet on sender side, then settles to receiver

**`Splice.AmuletAllocation:AmuletAllocation`** — Locks amulet for a trade/allocation; implements `allocation_executeTransferImpl`, `allocation_withdrawImpl`, `allocation_cancelImpl`

---

### **2\. Mining Round Lifecycle**

Rounds follow a strict state machine: **Open → Summarizing → Issuing → Closed**

**`Splice.Round:OpenMiningRound`** — Active round accepting reward registrations

* Fields: amulet price, opens/closes timing, issuance config, transfer fees in USD

* Choice: `OpenMiningRound_Fetch` (non-consuming read)

**`Splice.Round:SummarizingMiningRound`** — Transition state; no new reward registrations

**`Splice.Round:IssuingMiningRound`** — Distributes amulets to coupon holders

* Fields: per-coupon issuance amounts for validator, featured app, unfeatured app, SV, faucet categories

**`Splice.Round:ClosedMiningRound`** — Terminal record; used to expire stale reward coupons

Managed via `AmuletRules` choices:

* `AmuletRules_Bootstrap_Rounds` — genesis

* `AmuletRules_AdvanceOpenMiningRounds` — tick forward

* `AmuletRules_MiningRound_StartIssuing` — open → issuing

* `AmuletRules_MiningRound_Close` — close after expiry

---

### **3\. Reward Coupon Transactions**

These are created when activity occurs and consumed when rewards are claimed:

**`Splice.Amulet:AppRewardCoupon`** — For featured app providers; weight-based **`Splice.Amulet:ValidatorRewardCoupon`** — For validators based on user activity

**`Splice.Amulet:SvRewardCoupon`** — For Super Validator nodes; weight-based issuance shares 

**`Splice.Amulet:ValidatorFaucetCoupon`** — Deprecated; replaced by `ValidatorLivenessActivityRecord` 

**`Splice.Amulet:DevelopmentFundCoupon`** — Emissions to development fund beneficiaries (up to 20, weights must sum to 1.0) 

**`Splice.Amulet:FeaturedAppActivityMarker`** — Non-transfer activity tracking for featured apps 

**`Splice.Amulet:UnclaimedActivityRecord`** — Time-expiring minting authorizations **`Splice.Amulet:ValidatorRight`** — Rights for validators claiming burn-based issuances

All bulk-expired via `AmuletRules_ClaimExpiredRewards`.

---

### **4\. Payment Workflows**

**`Splice.Wallet.Payment:AppPaymentRequest`** — App initiates payment to multiple receivers

* Choices: `AppPaymentRequest_Accept` (converts USD→CC, creates LockedAmulet), `_Expire`, `_Withdraw`, `_Reject`

**`Splice.Wallet.Payment:AcceptedAppPayment`** — Locked escrow after acceptance

* Choices: `AcceptedAppPayment_Collect` (distributes to receivers), `_Reject` (refund), `_Expire` (reclaim after lock)

**`Splice.Wallet.Payment:TerminatedAppPayment`** — Tombstone contract; signals completion to coordinating workflows

---

### **5\. Subscription Workflows**

A state machine for recurring payments:

SubscriptionRequest

    → AcceptAndMakePayment → SubscriptionInitialPayment

        → Collect → Subscription \+ SubscriptionIdleState

            → MakePayment → SubscriptionPayment

                → Collect → SubscriptionIdleState (loop)

                → Reject/Expire → SubscriptionIdleState

            → ExpireSubscription / CancelSubscription

Key templates: `SubscriptionRequest`, `SubscriptionInitialPayment`, `Subscription`, `SubscriptionIdleState`, `SubscriptionPayment`

---

### **6\. DSO Governance Transactions**

**`Splice.DsoRules:DsoRules`** — Master governance contract

* `DsoRules_RequestVote` / `DsoRules_CastVote` — voting on actions

* `DsoRules_CloseVoteRequest` — close after 2/3+ super-majority

* `DsoRules_ConfirmAction` / `DsoRules_ExecuteConfirmedAction` — automated execution

* `DsoRules_AddSv` / `DsoRules_OffboardSv` / `DsoRules_UpdateSvRewardWeight` — validator management

**`Splice.DsoRules:VoteRequest`** — On-chain governance proposal with `targetEffectiveAt`

**`Splice.DsoRules:Confirmation`** — Automated execution when preconditions met

**`ActionRequiringConfirmation`** — Tagged union: `ARC_DsoRules`, `ARC_AmuletRules`, `ARC_AnsEntryContext`

---

### **7\. Amulet Name Service (ANS)**

**`Splice.Ans:AnsRules`** — Fee schedule and entry rules (DSO signatory)

* `AnsRules_RequestEntry` — user initiates name registration via subscription

* `AnsRules_CollectInitialEntryPayment` — creates active entry after first payment

* `AnsRules_CollectEntryRenewalPayment` — processes renewals

**`Splice.Ans:AnsEntry`** — Active name registration with expiry

* Choices: `AnsEntry_Renew`, `AnsEntry_Expire`

**`Splice.Ans:AnsEntryContext`** — Pending entry awaiting payment (signatories: DSO \+ user)

---

### **8\. Validator Management**

**`Splice.ValidatorLicense:ValidatorLicense`** — Establishes validator status (DSO signatory only; validator is observer)

* `ValidatorLicense_RecordValidatorLivenessActivity` — replaces deprecated faucet coupons

* `ValidatorLicense_UpdateMetadata` — version/contact updates (1-hour cooldown)

**`Splice.ValidatorLicense:ValidatorLivenessActivityRecord`** — Per-round activity record

**`Splice.ValidatorOnboarding:ValidatorOnboarding`** — Secret-based validator onboarding

* `ValidatorOnboarding_Match` — validates secret, creates `UsedSecret` (prevents replay)

* `ValidatorOnboarding_Expire`

**`Splice.DecentralizedSynchronizer:MemberTraffic`** — Tracks bandwidth purchases per member

* Fields: totalPurchased (bytes), numPurchases, amuletSpent, usdSpent

---

### **9\. Splitwell (Reference Application)**

A complete expense-splitting app showing full DAML patterns:

* **`SplitwellRules`** — hub contract with non-consuming choices delegating to groups

* **`Group`** — shared spending round; members are co-signatories

* **`BalanceUpdate`** — records external payment, peer transfer, or netting

* **`TransferInProgress`** — pending transfer with completion/cancellation
...
| Pattern | Example |
| :---- | :---- |
| **Two-step (propose/accept)** | `AppPaymentRequest` → `AcceptedAppPayment` |
| **State machine** | `OpenMiningRound` → `IssuingMiningRound` → `ClosedMiningRound` |
| **Hub contract** (non-consuming) | `AmuletRules`, `SplitwellRules` |
| **Tombstone/marker** | `TerminatedAppPayment`, `ClosedMiningRound` |
| **Escrow via lock** | `LockedAmulet` with `TimeLock` |
| **Secret-based auth** | `ValidatorOnboarding` \+ `UsedSecret` |
...
But instead of running against one database, different actors exchange which stored procedures they want to run via the Canton protocol. The protocol orders, validates, removes conflicts, and distributes these requests so all affected parties can apply them deterministically to their local stores.
...
* [SDK Components](https://docs.digitalasset.com/build/3.3/overview/sdk_components.html)
...
* [Smart Contract Upgrade](https://docs.daml.com/upgrade/smart-contract-upgrades.html)
